...
public class ObsidianAnvil extends Block
{
    ...
    public ActionResultType onBlockActivated(BlockState state, World worldIn, BlockPos pos, PlayerEntity player, Hand handIn, BlockRayTraceResult hit)
    {
        ...
        ItemStack shard = new ItemStack(ModItems.DIAMOND_SHARD.get(), 3);
        Item item = itemstack.getItem();
        BlockState anvil = worldIn.getBlockState(pos);
        BlockPos anvilPos = pos;

        if (item == Items.DIAMOND)
        {
            worldIn.setBlockState(anvilPos, anvil.with(ObsidianAnvil.STAGE, 1), 1);
            player.setHeldItem(handIn, ItemStack.EMPTY);
            return ActionResultType.SUCCESS;
        } else if (item == ModItems.OBSIDIAN_HAMMER.get())
        {
            worldIn.setBlockState(anvilPos, anvil.with(ObsidianAnvil.STAGE, 0), 1);
